...
const adminTableOrder = document.getElementById('adminOrder').getElementsByTagName("TBODY")[0];

//Drawing a table product with data from local storage
for ( let i = 0, len = localStorage.length; i < len; ++i ) {
	let returnObj = JSON.parse(localStorage.getItem( localStorage.key( i ) ));

	if(!returnObj.ordered){
...
//Drawing a table order with data from local storage
for ( let i = 0, len = localStorage.length; i < len; ++i ) {
	let returnObj = JSON.parse(localStorage.getItem( localStorage.key( i ) ));
	...
	if(returnObj.ordered){
		let tr = document.createElement('TR');
		let td1 = document.createElement('TD');
		td1.appendChild(document.createTextNode(returnObj.customer.name));
		let td2 = document.createElement('TD');
		td2.appendChild(document.createTextNode(returnObj.customer.surname));
		let td3 = document.createElement('TD');
		td3.appendChild(document.createTextNode(returnObj.customer.phone));
		let td4 = document.createElement('TD');
